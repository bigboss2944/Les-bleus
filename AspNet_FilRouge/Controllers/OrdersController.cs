...
namespace AspNet_FilRouge.Controllers
{
    ...
    public class OrdersController : Controller
    {
        ...
        {
            var bicycles = _db.Bicycles.Where(b => b.Order == null).ToList();
            ...
            long? CustomerId, long? ShopId, List<long>? BicycleIds)
        {
            if (ModelState.IsValid)
            {
                ...
                if (currentUser != null)
                {
                    order.Seller = await _db.Sellers.FirstOrDefaultAsync(s => s.Id == currentUser.Id);
                }

                if (CustomerId.HasValue)
                    order.Customer = await _db.Customers.FindAsync(CustomerId.Value);

                if (ShopId.HasValue)
                    order.Shop = await _db.Shops.FindAsync(ShopId.Value);
        ...
        public async Task<IActionResult> Edit(long? id)
        {
            ...
            ViewBag.Bicycles = _db.Bicycles.Where(b => b.Order == null || b.Order.IdOrder == id).ToList();
            ViewBag.Customers = _db.Customers.ToList();
            ViewBag.Shops = _db.Shops.ToList();
            return View(order);
        }
            ...
            long? CustomerId, long? ShopId)
        {
            if (ModelState.IsValid)
            {
                ...
                if (CustomerId.HasValue)
                    existing.Customer = await _db.Customers.FindAsync(CustomerId.Value);

                if (ShopId.HasValue)
                    existing.Shop = await _db.Shops.FindAsync(ShopId.Value);

                await _db.SaveChangesAsync();
                return RedirectToAction(nameof(Index));
            }

            ViewBag.Bicycles = _db.Bicycles.Where(b => b.Order == null || b.Order.IdOrder == order.IdOrder).ToList();
            ViewBag.Customers = _db.Customers.ToList();
            ViewBag.Shops = _db.Shops.ToList();
            return View(order);
        }
        ...
        {
            var order = await _db.Orders.Include(o => o.Bicycles).FirstOrDefaultAsync(o => o.IdOrder == orderId);
            ...
            var bicycle = await _db.Bicycles.FindAsync(bicycleId);
        ...
        public async Task<IActionResult> RemoveBicycle(long orderId, long bicycleId)
        {
            ...
            var bicycle = await _db.Bicycles.FirstOrDefaultAsync(b => b.Id == bicycleId && b.Order != null && b.Order.IdOrder == orderId);
