...
public class StockRequestRepositoryTests
{
    ...
    public async Task CreateStockRequest_DefaultStatusIsEnAttente()
    {
        ...
        var request = new VendeurModels.StockRequest
        {
    ...
    {
        using var context = CreateContext();
        var request = new VendeurModels.StockRequest { BicycleName = "Trek FX3", Quantity = 3 };
        context.StockRequests.Add(request);
        await context.SaveChangesAsync();
    ...
    {
        using var context = CreateContext();
        var request = new VendeurModels.StockRequest { BicycleName = "Specialized Allez", Quantity = 2 };
        context.StockRequests.Add(request);
        await context.SaveChangesAsync();
    ...
    public async Task RetrieveAllStockRequests_OrderedByDate()
    {
        ...
        context.StockRequests.AddRange(
            new VendeurModels.StockRequest { BicycleName = "A", Quantity = 1, RequestDate = now.AddDays(-2) },
            new VendeurModels.StockRequest { BicycleName = "B", Quantity = 2, RequestDate = now.AddDays(-1) },
            new VendeurModels.StockRequest { BicycleName = "C", Quantity = 3, RequestDate = now });
        await context.SaveChangesAsync();
