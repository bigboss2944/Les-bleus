        ...
        private async Task SyncAsync()
        {
            ...
            try
            {
                ...
                _logger.LogInformation(
                    "Synchronisation Vendeur: {Orders} ordres, {Bicycles} vélos, {Sellers} vendeurs, {Customers} clients, {StockRequests} demandes stock.",
                    orders.Count, bicycles.Count, sellers.Count, customers.Count, stockRequests.Count);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Erreur dans SyncAsync du vendeur");
                throw;
            }
        }
    }
